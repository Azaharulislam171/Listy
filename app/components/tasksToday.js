...
const TasksToday = () => {
    ...
    useEffect(() => {
      const fetchTasks = async (uid) => {
        ...
        // Adjust for UTC-4 (subtract 4 hours)
        const startOfDayTimestamp = Timestamp.fromMillis(today.getTime() - 4 * 60 * 60 * 1000);
        const endOfDayTimestamp = Timestamp.fromMillis(today.getTime() + 24 * 60 * 60 * 1000 - 4 * 60 * 60 * 1000 - 1);
  
        const q = query(
          collection(db, 'tasks', uid, 'userTasks'),
          where('taskStartDate', '<=', endOfDayTimestamp),
                      ...
                      <thead className="bg-gray-50">
                        <tr>
                          <th scope="col" className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                            <span className="sr-only text-gray-500">Action</span>
                          </th>
                          <th scope="col" className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                            Title
                          </th>
